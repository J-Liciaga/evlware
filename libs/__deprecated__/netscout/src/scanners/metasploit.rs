...
impl MetasploitScanner {
    pub async fn new(url: &str) -> Result<Self> {
        let addr = url.to_socket_addrs()?.next().context("Invalid address")?;
    ...
    async fn get_exploit_suggestions(&self, target: &str) -> Result<Vec<ExploitSuggestion>> {
        let params = (self.token.clone(), "exploit", target);
    ...
    async fn run_exploit(&self, exploit: &str, target: &str) -> Result<Vec<Vulnerability>> {
        let params = (self.token.clone(), exploit, target);
        let result: Vec<String> = self.client.call("module.execute", &params)
            .await
            .context("Failed to execute exploit")?;
